...
wchar_t* vowSort(struct Sentence sent){
    wchar_t** words = (wchar_t**) malloc(sizeof(wchar_t*));     // ВЫДЕЛЕННАЯ ПАМЯТЬ WORDS

    wchar_t* buffer = malloc(sent.memSentence*sizeof(wchar_t)); // выделенная память buffer
    wcscpy(buffer, sent.sentence);
    void* bufferPoint = buffer;                                 // BUFFERPOINT адресс с которого ВЫДЕЛЕНА ПАМЯТЬ ПОД buffer
...
struct Sentence strPattern (struct Sentence sent){
    ...
    for(count = 0; word; count++){
        ...
        // Выделение маски
    wchar_t* mask = calloc(mlen+3, sizeof(wchar_t));    // выделена память mask, очистка не требуется
    wcscpy(mask, words[0]);
...
wchar_t* getMask(wchar_t* mask, wchar_t* word, int maxlen){
    ...
    for(int i = -word_len + 1; /*i < 1*/ i < mask_len; i++){
        lstar_tmp = 0;
        rstar_tmp = 0;
        wchar_t* upStr = calloc((maxlen+1), sizeof(wchar_t));        // ВЫДЕЛЕННАЯ ПАМЯТЬ UPSTR (в цикле)
        wchar_t* dwStr = calloc((maxlen+1), sizeof(wchar_t));        // ВЫДЕЛЕННАЯ ПАМЯТЬ DWSTR (в цикле)
        if (i < 1){
        ...
        wchar_t* tmpMask = malloc((maxlen+3)*sizeof(wchar_t));      // ВЫДЕЛЕННАЯ ПАМЯТЬ TMPMASK (в цикле)
        int congs = 0;
        ...
        free(upStr);        // UPSTR ОЧИЩЕНА
        free(dwStr);        // DWSTR ОЧИЩЕНА
        free(tmpMask);      // TMPMASK ОЧИЩЕНА
    }
    ...
    free(emptyMask);        // EMPTYMASK ОЧИЩЕНА
    free(befTmp);           // BEFTMP ОЧИЩЕНА
    free(aftTmp);           // AFTTMP ОЧИЩЕНА
    free(unstarMask);       // UNSTARMASK ОЧИЩЕНА
    return outMask;
}
