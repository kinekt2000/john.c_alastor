...
int strangeCmp(const void* a, const void* b){
    struct Sentence aa = *(struct Sentence*)a;
    struct Sentence bb = *(struct Sentence*)b;

    int indA = 0;
    for(int i = 0; i < aa.sizeSentence; i++){
        if (iswalpha(aa.sentence[i])) indA ++;
        if (iswdigit(aa.sentence[i])) indA --;
    }

    int indB = 0;
    for(int i = 0; i < bb.sizeSentence; i++){
        if (iswalpha(bb.sentence[i])) indB ++;
        if (iswdigit(bb.sentence[i])) indB --;
    }
...
int strangeSort(const struct Text input, struct Text* output){
    output->memText = input.memText;
    output->sizeText = input.sizeText;
    wprintf(L"Выделение памяти: ");
    struct Sentence* buffer = malloc(output->memText*sizeof(struct Sentence));
    ...
    wprintf(L"Форматирование: ");
    for(int i = 0; i < output->sizeText; i++){
        output->text[i].sentence = malloc(input.text[i].memSentence * sizeof(wchar_t));
        wcscpy(output->text[i].sentence, input.text[i].sentence);
        output->text[i].memSentence = input.text[i].memSentence;
        output->text[i].sizeSentence = input.text[i].sizeSentence;
    }

    qsort(output->text, output->sizeText - 1, sizeof(struct Sentence), strangeCmp);
    wprintf(L"Выполнено\n");
    return 0;
}
